...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    out, x, prev_h, Wx, Wh, b = cache
    dout = 1.0 / np.cosh(out)**2 * dnext_h
    dx = dout @ Wx.T
    dprev_h = dout @ Wh.T
    dWx = x.T @ dout
    dWh = prev_h.T @ dout
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      step_cache = cache[t+4]
      dnext_h = dh[:,t,:] + dprev_h
      dxs, dprev_h, dWxs, dWhs, dbs = rnn_step_backward(dnext_h, step_cache)
      dx[:,t,:] += dxs
      if t == 0:
        dh0 = dprev_h
      dWx += dWxs
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c_local = dnext_c + o * dnext_h * 1.0 / np.cosh(next_c)**2
    dprev_c = dnext_c_local * f
    df = dnext_c_local * prev_c
    di = dnext_c_local * g
    dg = dnext_c_local * i
    dai = di * i * (1 - i)
    daf = df * f * (1 - f)
    dao = do * o * (1 - o)
    dag = dg * 1.0 / np.cosh(ag)**2
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      ...
      dnext_h = dh[:,t,:] + dprev_h
      dnext_c = dprev_c
      dxs, dprev_h, dprev_c, dWxs, dWhs, dbs = lstm_step_backward(dnext_h, dnext_c, step_cache)
      dx[:,t,:] = dxs
      if t == 0:
        dh0 = dprev_h
      dWx += dWxs
